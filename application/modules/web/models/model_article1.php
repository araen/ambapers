...
class Model_article extends CI_Model {
	...
	function get_by_category($page=0,$limit=20, $c=''){
		$this->load->database();
		
		$page = $page*$limit;

		$sql = "SELECT articles.*, categories.name FROM articles LEFT JOIN categories ON id_category=categories.id WHERE (categories.permalink='$c' OR categories.parent='$c') ORDER BY articles.id DESC LIMIT $page,$limit;";
		$query['data'] = $this->db->query($sql);
		
		$sql = "SELECT articles.*, categories.name FROM articles LEFT JOIN categories ON id_category=categories.id WHERE (categories.permalink='$c' OR categories.parent='$c')";
		$query['count'] = $this->db->query($sql);
		$this->db->close();
