...
class Model_comment extends CI_Model {
	...
	function get_by_link($link,$published=''){
		$this->load->database();
			$filter='';
			if($published!='')
				$filter .= " AND articles.published='$published'";	
			$sql = "SELECT comments.* FROM comments JOIN articles ON id_article=articles.id WHERE articles.permalink='$link' $filter ORDER BY id;";
			$query = $this->db->query($sql);
